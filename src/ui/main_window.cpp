...
void MainWindow::OpenRepository(const std::string& path)
{
    auto repo = GitRepository::Open(path);
    SetRepository(repo);
}

void MainWindow::SetRepository(std::shared_ptr<GitRepository> repo)
{
    m_repository = repo;
    if (m_statusPanel) {
        m_statusPanel->SetRepository(repo);
    }
    if (repo) {
...
void MainWindow::RenderCenterPanel()
{
    ImGui::Begin("Commits", nullptr, ImGuiWindowFlags_NoCollapse);
    if (!m_repository) {
        ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "Open a repository to view commit history");
    } else {
        ImGui::Text("Commit history will be displayed here");
    }
    ImGui::End();
}
...
{
    ImGui::Begin("Diff View", nullptr, ImGuiWindowFlags_NoCollapse);
    if (!m_repository) {
        ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "Select a commit to view diff");
    } else {
        ImGui::Text("Diff content will be displayed here");
    }
    ImGui::End();
}
...
void MainWindow::OnEvent(const SDL_Event& event)
{
    if (event.type == SDL_EVENT_KEY_DOWN) {
        if (event.key.key == SDLK_ESCAPE) {
            Quit();
        }
        if (event.key.key == SDLK_F5 && m_statusPanel) {
